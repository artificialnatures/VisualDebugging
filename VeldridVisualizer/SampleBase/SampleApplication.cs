...
namespace SampleBase
{
    public abstract class SampleApplication
    {
        ...
        public Stream OpenEmbeddedAssetStream(string name)
        {
            ...
            var matchingName = assembly.GetManifestResourceNames().First(rn => rn.Contains(name));
            return assembly.GetManifestResourceStream(matchingName);
        }

        public Shader LoadShader(ResourceFactory factory, ShaderStages stage, string entryPoint)
        {
            string name = $"{stage.ToString()}.{GetExtension(factory.BackendType)}";
